...
class APIClient(object):
    ...
    def get_realtime_ticker(self, callback):
        req = PricingStream(accountID=self.account_id, params={"instruments": settings.product_code})
        try:
    ...
    def send_order(self, order: Order) -> Trade:
        if order.side == constants.BUY:
            ...
            side = -1
        order_data = {
            "order": {"type": order.order_type, "instrument": order.product_code, "units": order.units * side}
        }
