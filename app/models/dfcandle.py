...
class Macd(Serializer):
    def __init__(
        self, fast_period: int, slow_period: int, signal_period: int, macd: list, macd_signal: list, macd_hist: list
    ):
...
class DataFrameCandle(object):
    ...
    def back_test_ichimoku(self):
        ...
        for i in range(1, len(self.candles)):
            if (
                chikou[i - 1] < self.candles[i - 1].high
            ...
            if (
                chikou[i - 1] > self.candles[i - 1].low
    ...
    def back_test_macd(self, macd_fast_period: int, macd_slow_period: int, macd_signal_period: int):
        if (
            len(self.candles) <= macd_fast_period
            or len(self.candles) <= macd_slow_period
            or len(self.candles) <= macd_signal_period
        ):
            return None
